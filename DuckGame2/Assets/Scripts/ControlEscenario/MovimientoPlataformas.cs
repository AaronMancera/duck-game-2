...
public class MovimientoPlataformas : MonoBehaviour
{
    ...
    private bool direccionContraria;
    ...
    void MoverPlataforma() 
    {
        ...
        //float limitadorMovimiento2 = limitadorMovimiento;
        //limitadorMovimiento2 -= Mathf.Abs(velocidadMovimiento * Time.deltaTime);

        //Debug.Log("limitadr : " + limitadorMovimiento2 + "posicion: " + posicionActual + "posicion2: "+ plataformas[plataformaActual].position);
        // Si la plataforma alcanza la posición yDestino
        if (direccionElegida == "arriba" && posicionActual.y > posicionesIniciales[plataformaActual].y + limitadorMovimiento || direccionElegida == "abajo" && posicionActual.y < posicionesIniciales[plataformaActual].y -limitadorMovimiento)
            {
        ...
        else if (direccionElegida == "derecha" && posicionActual.x > posicionesIniciales[plataformaActual].x + limitadorMovimiento || direccionElegida == "izquierda" && posicionActual.x < posicionesIniciales[plataformaActual].x - limitadorMovimiento)
            {
                //Debug.Log("cambio dir");
